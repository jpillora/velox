...
(function(window, document) {
  ...
  function Velox(type, url, obj) {
    switch (type) {
      case velox.WS:
        if (!window.WebSocket) throw "This browser does not support WebSockets";
        this.ws = true;
        break;
      case velox.SSE:
        this.sse = true;
  ...
  Velox.prototype = {
    connect: function() {
      if (vs.indexOf(this) === -1) vs.push(this);
      this.retrying = true;
      this.retry();
    },
    retry: function() {
    ...
    connmessage: function(event) {
      ...
      //perform update
      if (update.delta) jsonpatch.apply(this.obj, update.body);
      else merge(this.obj, update.body);
      //auto-angular
